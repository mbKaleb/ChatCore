//
//  SelectableMarkdownDemo2.swift
//  ChatCore
//
//  Prototype harness. Open the previews, or drop `SelectableMarkdownDemo2()` into
...
let selectableKitchenSink2 = """
	...
	```swift
	// The card behind this is painted by SelectableLayoutFragment2,
...
struct SelectableMarkdownDemo2: View {

	@State private var fontSize: Double = 14
	@State private var showsSecondMessage = true

	private var style: Selectable2Style {
		...
		VStack(spacing: 0) {
			...
			ScrollView {
				VStack(alignment: .leading, spacing: 20) {
					message(selectableKitchenSink2)
	...
	private func message(_ markdown: String) -> some View {
		SelectableMarkdownView2(markdown: markdown, style: style)
			.padding(14)
			.background {
...
#Preview("Selectable markdown") {
	SelectableMarkdownDemo2()
		.frame(width: 620, height: 780)
}

#Preview("Single message, dark") {
	ScrollView {
		SelectableMarkdownView2(markdown: selectableKitchenSink2)
			.padding(20)
	}
