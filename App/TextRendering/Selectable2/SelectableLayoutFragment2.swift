//
//  SelectableLayoutFragment2.swift
//  ChatCore
//
...
nonisolated final class SelectableLayoutFragment2: NSTextLayoutFragment {
	...
	override func draw(at point: CGPoint, in context: CGContext) {
		...
		// After `super`, so an equation paints over the (empty) attachment glyph
	...
	private func drawInlineCodePills(at point: CGPoint, in context: CGContext) {
		...
		for line in textLineFragments {
			...
			string.enumerateAttribute(.inlineCode2, in: range) { value, subrange, _ in
	...
	private func drawMath(at point: CGPoint, in context: CGContext) {
		for line in textLineFragments {
			...
			string.enumerateAttribute(.attachment, in: range) { value, subrange, _ in
				guard
					let attachment = value as? MathAttachment2,
