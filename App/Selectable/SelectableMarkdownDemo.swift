//
//  SelectableMarkdownDemo.swift
//  ChatCore
//
//  Prototype harness. Open the previews, or drop `SelectableMarkdownDemo()` into
...
let selectableKitchenSink = """
	...
	```swift
	// The card behind this is painted by SelectableLayoutFragment,
	...
	it as a single character rather than stopping at it.
...
struct SelectableMarkdownDemo: View {

	@State private var fontSize: Double = 14
	@State private var showsSecondMessage = true

	private var style: SelectableStyle {
	...
	private func message(_ markdown: String) -> some View {
		SelectableMarkdownView(markdown: markdown, style: style)
			.padding(14)
			.background {
...
#Preview("Selectable markdown") {
	SelectableMarkdownDemo()
		.frame(width: 620, height: 780)
}

#Preview("Single message, dark") {
	ScrollView {
		SelectableMarkdownView(markdown: selectableKitchenSink)
			.padding(20)
	}
